...
@Entity
@DiscriminatorValue("Insecto")
class Insecto() : Vector() {
    ...
    override fun puedeContagiarSegunVector(vector: Vector, especieAContagiar: Especie): Boolean {
        return this.tieneComodin(vector.getTipoVector(), especieAContagiar) || vector.getTipoVector() != "Insecto"
    }

    override fun caminosRecorribles(): List<String> {
        return mutableListOf("Terrestre", "Aereo")
    }
}